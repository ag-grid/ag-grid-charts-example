...
document.addEventListener('DOMContentLoaded', () => {
    ...
    svgPath.fillStyle = '#faeb00';
    svgPath.strokeStyle = 'black';
    svgPath.lineWidth = 3;
    svgPath.scalingX = 4;
    svgPath.scalingY = 4;
    svgPath.path = starPath;

    rootGroup.append([svgPath]);
    scene.root = rootGroup;

    function animateLineDashOffset() {
        svgPath.lineDash = [3, 3];
        svgPath.lineDashOffset = 0;
        return new Promise(resolve => {
            function step() {
                svgPath.lineDashOffset += 0.25;
                if (svgPath.lineDashOffset > 50) {
                    resolve();
                    return;
                }
    ...
    delay().then(() => {
        svgPath.lineJoin = 'round';
    }).then(delay).then(() => {
        svgPath.lineJoin = 'bevel';
    }).then(delay).then(() => {
        svgPath.lineJoin = null;
    }).then(delay).then(() => {
        svgPath.lineDash = [4, 4];
    }).then(delay).then(() => {
        svgPath.lineCap = 'round';
    }).then(delay).then(() => {
        svgPath.lineJoin = 'round';
    }).then(delay).then(() => {
        svgPath.translationX = 20;
        svgPath.translationY = 20;
        svgPath.scalingX = 2;
        svgPath.scalingY = 2;
        svgPath.path = dogPath;
    }).then(animateLineDash).then(animateLineDashOffset)
        .then(() => {
            // Test rotation and scaling centers
            // by rotating, scaling and fading away.
            svgPath.rotationCenterX = 300;
            svgPath.rotationCenterY = 185;
            svgPath.scalingCenterX = 300;
            svgPath.scalingCenterY = 185;
            function step() {
                svgPath.rotation += Math.PI / 10;
                svgPath.scalingX -= 0.05;
                svgPath.scalingY -= 0.05;
                svgPath.opacity -= 0.025;
                if (svgPath.opacity <= 0) {
                    return;
                }
                requestAnimationFrame(step);
            }
